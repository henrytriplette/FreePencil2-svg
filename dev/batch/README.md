...
各モデルで SVG 書き出しも1回通し、プロッタ向けの実測値を採る
（`--no-svg` で省ける）。ラスタの ink/components が「線が出ているか」を
見るのに対し、こちらは「実際に引けるか」を見る。

- **本数** — 書き出されたパスの数。ペンの上げ下ろしの回数に直結する
- **描画 / 移動** — ペンを下ろして描く距離と、上げて移動する距離
- **移動比** — 移動 ÷ 描画。低いほど無駄がない
- **目安** — 所要時間の見積り（ペン速度などの既定値による）
- **merge_ratio** — 結合後の本数 ÷ 結合前。低いほどよく繋がっている

**スコアには入れていない。** 過去のレポートと比べられなくなるため、
スコアの式（線密度45% + 断片化30% + 塗り分け違反25%）は据え置き。

SVG 自体は `out/svg/` に残り、レポートの score 欄からリンクされる。
